...
\paragraph{Surface Area of Revolution} Given the curve $C: y = f(x)$, the surface area of the curve between two points $a$ and $b$, where $y'(x)$ is continuous, we would like the compute the surface area of the curve:
    $$ A = \int_C 2\pi y \, ds$$
\paragraph{Formula} Deriving the formula for the surface area of a curve $y = f(x)$ between two points $a$ and $b$ is similar to the arc length formula:
\begin{align}
    \intertext{At $x_i$ and $x_{i+1}$, the length of the segment is:}
