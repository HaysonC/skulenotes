...
\begin{definition}[Undamped Forced Oscillations]
    Consider a driver porves on the left side of the sping, we have the following differential equation:
    \begin{equation}
        m\ddot{x} + kx = \eta
    \end{equation}
    where \( \eta = F_0 \cos(\omega t) \) is the driving force with amplitude \( F_0 \) and angular frequency \( \omega \). The general solution to this non-homogeneous differential equation given by:
    \begin{equation}
        x(t) = A(\omega)\cos(\omega t - \delta) 
    \end{equation}
    to derive $A(\omega)$ and $\delta$, we can use the equation of motion:
    \begin{align*}
        A(\omega)(-\omega^2 + \omega_0^2) \cos(\delta) = \omega_0^2 a
    \end{align*}
    where \( a = \frac{F_0}{k} \) is the static displacement of the mass when the driving force is constant. We also haev:
    \begin{align*}
